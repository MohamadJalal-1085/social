// Sample post data with refined descriptions and random comments
const samplePosts = [
    {
        id: '1',
        author: {
                                ...
                                <div class="comment">
                                    ...
                                    <div class="comment-content">
                                        ...
                                        <p class="comment-time">
                                            ${comment.author === 'You' && state.editingCommentId !== comment.id ?
                                            `<span><button class="edit-btn" onclick="editComment('${post.id}', '${comment.id}')">Edit</button> • <button class="delete-btn" onclick="deleteComment('${post.id}', '${comment.id}')">Delete</button></span>` : '<span></span>'}
...
function openImageModal(imageSrc) {
    ...
    modal.innerHTML = `
        <div class="image-modal-content">
            <img src="${imageSrc}" alt="Enlarged post image" class="image-modal-img" onclick="toggleZoom(this)">
...
function toggleMenu(postId) {
    Object.keys(postStates).forEach(id => {
        if (id !== postId) {
            postStates[id].menuOpen = false;
        }
    });
...
function reportUser(postId) {
    alert('User reported');
    postStates[postId].menuOpen = false;
    renderPosts();
}

function notInterested(postId) {
    alert('Marked as not interested in content');
    postStates[postId].menuOpen = false;
    renderPosts();
}
...
function addComment(postId) {
    ...
    if (content) {
        ...
        renderPosts();
        setTimeout(() => {
            const commentsList = document.querySelector(`#comment-input-${postId}`).closest('.comments-section').querySelector('.comments-list');
...
function followSuggestion(button) {
    if (button.textContent === 'Follow') {
        ...
        button.style.backgroundColor = 'rgba(13, 148, 136, 0.1)';
        button.style.color = '#0d9488';
        button.style.borderColor = 'rgba(13, 148, 136, 0.2)';
    } else {
...
function refreshPosts() {
    ...
    setTimeout(() => {
        currentPosts = [...samplePosts].sort(() => Math.random() - 0.5);
...
document.addEventListener('click', function(event) {
    if (!event.target.closest('.menu-btn') && !event.target.closest('.menu-dropdown')) {
        let anyMenuClosed = false;
        Object.keys(postStates).forEach(id => {
            if (postStates[id].menuOpen) {
                postStates[id].menuOpen = false;
                anyMenuClosed = true;
            }
        });
